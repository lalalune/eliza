...
```bash
uv run pytest packages/training/scripts/quantization/test_recipes_smoke.py packages/training/scripts/quantization/test_gguf_eliza1_apply.py -q
```

Result: `45 passed, 1 skipped`.

```bash
uv run python -m compileall -q packages/training/scripts/quantization packages/training/scripts/turn_detector/convert_to_gguf.py packages/training/scripts/run_pipeline.py
```

Result: passed.

```bash
uv run python packages/training/scripts/quantization/gguf-q4_k_m_apply.py --model google/gemma-4-E2B --output /tmp/eliza-stage3-q4-dry --dry-run
```
...
uv run python packages/training/scripts/quantization/gguf_eliza1_apply.py --checkpoint "$tmpdir/checkpoint" --output "$tmpdir/model.gguf" --outtype q8_0 --llama-cpp-dir "$tmpdir/llama.cpp" --dry-run
```
