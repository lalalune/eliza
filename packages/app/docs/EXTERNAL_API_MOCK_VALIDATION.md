...
fixtures can silently drift. `test/external-api-mock-validation.test.ts`
discovers provider routes directly from the UI-smoke TypeScript AST and
requires each provider to have real validation artifacts or a reasoned
exemption.
...
4. `src/routes.real.test.ts` is environment-gated and checks the live provider
   for drift.
5. The UI-smoke fixture produces the same contract-shaped DTO through a
   provider route recognized by the source-derived authority.
...
## Current source-discovered providers
...
The validation test enumerates `page.route` calls before classifying recognized
provider patterns. Comments and classification records do not participate in
discovery. It enforces:
