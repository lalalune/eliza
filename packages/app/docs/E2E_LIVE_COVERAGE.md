...
| `android-device-e2e` / `native-plugin-androidtest` | Android assistant + native plugin surfaces | runs every native plugin's instrumentation against an API 34 emulator, then reapplies and verifies the real assistant role, VoiceInteractionService, assist key, selected Eliza IME, focused-editor IME state, and unavailable-state deep link; uploads raw Android test reports, JSON/logcat/package-manager receipts, screenshot, and screenrecord | `ci:device` PR label + dispatch |
