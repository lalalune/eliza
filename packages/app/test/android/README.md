...
| `touch-gesture.android.spec.ts` | The installed Android WebView runs the full chat gesture matrix — sheet detents, combined-home inert horizontal swipes + embedded launcher grid reveal, push-to-talk hold, keyboard avoidance, media attachment, long-press — via real OS touch (`adb input`), asserting real touch delivery (never mouse) plus each gesture's semantics, recorded as one chunked screenrecord | Playwright Android driver + `adb shell input swipe` |
...
- **Combined home** (#16764 — no home↔launcher rail) — horizontal swipes over
  the embedded grid are navigation-inert (`home-launcher-surface[data-page]`
  stays `home`, no ghost tile launch, chat stays closed); launcher tiles are
  reached by scrolling the embedded grid (`home-apps-scroll` →
