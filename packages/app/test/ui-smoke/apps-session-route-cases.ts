...
export const DIRECT_ROUTE_CASES: readonly DirectRouteCase[] = [
  ...
  // spatial views. Anchor readiness to controls their current wrappers render
  // unconditionally, proving the real bundle mounted rather than relying on
  // the deleted toolbar aria-labels.
  {
    name: "hyperliquid",
    path: "/hyperliquid",
    readyChecks: [{ selector: '[data-agent-id="hyperliquid-refresh"]' }],
    timeoutMs: 90_000,
  },
  {
    name: "polymarket",
    path: "/polymarket",
    readyChecks: [{ selector: '[data-agent-id="polymarket-refresh"]' }],
    timeoutMs: 90_000,
  },
  {
  ...
  {
    name: "phone companion special-window shell",
    // The companion is a dedicated window shell selected by `mode=companion`,
    // not a normal dashboard tab. Without the mode the main shell correctly
    // falls back to the Views manager instead of mounting native companion UI.
    path: "/phone-companion?mode=companion",
    readyChecks: [{ text: "Companion" }, { text: "Pair" }],
    timeoutMs: 90_000,
  },
  {
