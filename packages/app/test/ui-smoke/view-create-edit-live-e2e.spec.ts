...
async function activateConversation(page: Page): Promise<ConversationSummary> {
  const response = await page.request.post("/api/conversations", {
    data: { title: "live view create/edit proof" },
  });
  expect(response.ok()).toBe(true);
  const body = (await response.json()) as {
  ...
  await seedAppStorage(page, {
    "eliza:chat:activeConversationId": id as string,
  });
  await openAppPath(page, "/chat");
  return { id: id as string, roomId: roomId as string };
}
...
test("creates, renders, edits, and hot-reloads a real plugin view", async ({
  ...
  try {
    ...
    await createdSurface.getByText(createdMarker).scrollIntoViewIfNeeded();
    ...
    await editedSurface.getByText(editedMarker).scrollIntoViewIfNeeded();
    await page.waitForTimeout(300);
