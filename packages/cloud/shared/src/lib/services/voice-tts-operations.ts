...
import {
  type VoiceTtsOperation,
  voiceTtsOperations,
} from "../../db/schemas/voice-tts-operations";
...
export function normalizeVoiceTtsIdempotencyKey(
  rawKey: string | null,
): string | null {
...
export class VoiceTtsOperationsService {
  async claim(params: {
    ...
    const [created] = await dbWrite
      ...
      .onConflictDoNothing({
        target: [
          voiceTtsOperations.organization_id,
          voiceTtsOperations.idempotency_key_hash,
        ],
      })
      .returning();
    ...
    if (existing.expires_at <= now) {
      await dbWrite
        ...
        .where(
          and(
            eq(voiceTtsOperations.id, existing.id),
            lte(voiceTtsOperations.expires_at, now),
          ),
        );
  ...
  async attachReservation(
    ...
    const [updated] = await dbWrite
      ...
      .where(
        and(
          eq(voiceTtsOperations.id, operationId),
    ...
    try {
      ...
      try {
        ...
        logger.warn("[VoiceTtsOperations] Failed to delete unreferenced result", {
          ...
          errorType:
            deleteError instanceof Error ? deleteError.name : "unknown",
        });
      }
      throw error;
    }
  ...
  async cleanupExpired(
    ...
    for (const operation of expired) {
      ...
      const rows = await dbWrite
        ...
        .where(
          and(
            eq(voiceTtsOperations.id, operation.id),
            lte(voiceTtsOperations.expires_at, now),
          ),
        )
        .returning({ id: voiceTtsOperations.id });
