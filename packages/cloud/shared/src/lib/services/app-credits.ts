...
export class AppCreditsService {
  ...
  async processPurchase(params: AppCreditPurchaseParams): Promise<AppCreditPurchaseResult> {
    ...
    if (
      ...
      new Decimal(creatorEarnings).greaterThan(
        persistedPurchaseAmount.minus(platformOffset),
      ) ||
  ...
  async deductCredits(params: AppCreditDeductionParams): Promise<AppCreditDeductionResult> {
    ...
    const pinnedCreatorUserId =
      typeof persistedMetadata.creatorUserId === "string"
        ? persistedMetadata.creatorUserId
        : null;
    ...
    const expectedTotalCost = new Decimal(persistedBaseCost)
      .plus(creatorMarkup)
      .toDecimalPlaces(6);
    ...
    if (orgDeduct.success) {
      ...
      const overageMetadata =
        overageDebit?.metadata && typeof overageDebit.metadata === "object"
          ...
          : {};
      const overageAmount = overageDebit
        ? Math.abs(Number(overageDebit.amount))
        : Number.NaN;
      if (
        !overageDebit ||
