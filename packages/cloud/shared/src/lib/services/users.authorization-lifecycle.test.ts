...
describe("UsersService strong authorization lifecycle", () => {
  ...
  test("logout publishes a monotonic JWT not-before revision before commit", async () => {
    ...
    const updated = await withStrongBoundary(() => usersService.revokeInferenceSessions(USER_ID));

    expect(updated.inference_session_not_before).toBeGreaterThan(issuedBeforeLogout);
  ...
  test("replaying logout cannot ratchet session denial beyond wall-clock time", async () => {
    ...
    const first = await withStrongBoundary(() => usersService.revokeInferenceSessions(USER_ID));
    const second = await withStrongBoundary(() => usersService.revokeInferenceSessions(USER_ID));
    ...
    expect((await readUser())?.inference_session_not_before).toBe(existingNotBefore);
  });
  ...
  test("a Steward identity change cannot commit before its newer user revision", async () => {
    ...
    await expect(
      withStrongBoundary(() => usersService.update(USER_ID, { steward_user_id: "steward-new" })),
    ).rejects.toMatchObject({
      code: "INFERENCE_AUTHORIZATION_BOUNDARY_REJECTED",
    });
