...
describe("shared conversation coordinator", () => {
  test("routes bridge, stream, and history through one room object", async () => {
    ...
    expect(
      (await coordinateSharedBridge(agent, rpc, { namespace })).result?.text,
    ).toBe("coordinated");
    expect(
      await (await coordinateSharedStream(agent, rpc, { namespace }))?.text(),
    ).toContain("event: done");
    expect(await coordinateSharedHistory("agent-1", "room-1", { namespace }))
      .toEqual([{ role: "assistant", content: "cached" }]);
    ...
    expect(envelopes.map((value) => (value as { operation: string }).operation))
      .toEqual(["bridge", "stream", "history"]);
