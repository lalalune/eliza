...
describe("SharedRuntimeChatService", () => {
  test("handles status, unknown methods, and invalid message input", async () => {
    ...
    expect(
      (await service.bridge(agent, { ...rpc, method: "heartbeat" })).result,
    ).toMatchObject({ ready: true, runtime: "shared" });
    expect(
      (await service.bridge(agent, { ...rpc, method: "unknown" })).error?.code,
    ).toBe(-32601);
    expect(
      (
  ...
  test("degraded and failed turns release admission at zero", async () => {
    ...
    expect((await service.bridge(agent, rpc, harness())).result?.degraded).toBe(
      true,
    );
    ...
    await expect(service.bridge(agent, rpc, harness())).rejects.toThrow(
      "turn failed",
    );
  ...
  test("translates insufficient admission to the bridge credit code", async () => {
    ...
    expect((await service.bridge(agent, rpc, harness())).error?.code).toBe(
      -32002,
    );
  });
