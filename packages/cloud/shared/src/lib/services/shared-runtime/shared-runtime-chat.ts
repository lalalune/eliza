...
async function characterFor(
  ...
  if (options.cacheOnly && agent.character_id && !linked) {
    ...
    const hydration = import("../characters/characters")
      .then(({ charactersService }) => charactersService.getById(characterId))
      ...
      .catch((error) => {
        ...
        logger.warn("[shared-runtime] character hydration failed", {
          agentId: agent.id,
          characterId,
