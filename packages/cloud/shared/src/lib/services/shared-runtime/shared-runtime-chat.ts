...
import { BRIDGE_INSUFFICIENT_CREDITS_CODE } from "../eliza-sandbox";
...
function channelId(agentId: string, params: Record<string, unknown>): string {
  const room =
    stringValue(params.roomId) ?? stringValue(params.userId) ?? "default";
...
async function loadHistory(
  ...
  const history = store
    ...
    : await sharedRuntimeHistoryRepository.get(agentId, roomId);
...
async function characterFor(
  ...
  const linked = agent.character_id
    ? cacheOnly
      ...
      : await import("../../../db/repositories/characters").then(
    ...
    : undefined;
  if (cacheOnly && agent.character_id && !linked) {
    throw new Error("[shared-runtime] linked character cache is warming");
  }
...
export class SharedRuntimeChatService {
  ...
  async bridge(
    ...
    const [character, history] = await Promise.all([
      characterFor(agent, Boolean(options.historyStore)),
