...
describe("creator movement retry healing", () => {
  ...
  test("purchase retry pays the charge-time owner after ownership changes", async () => {
    ...
    try {
      await expect(purchase()).rejects.toThrow(
        "simulated purchase payout acknowledgement loss",
      );
      const [successorOrg] = await dbWrite
        .insert(organizations)
  ...
  test("atomic projection acknowledgement loss replays without moving any total twice", async () => {
    ...
    try {
      await expect(deduct()).rejects.toThrow(
        "simulated projection commit acknowledgement loss",
      );
  ...
  test("sub-ledger-unit creator movements still pair their platform delta exactly", async () => {
    if (!pgliteReady) return;
    const { appId, payerOrganizationId, payerUserId, creatorUserId } = await seed();
    ...
    expect(await payerBalance(payerOrganizationId)).toBeCloseTo(99.999998, 9);
