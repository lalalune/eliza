...
async function postGate(
  ...
  return await stub.fetch(`https://anonymous-chat-gate${path}`, {
    method: "POST",
...
export async function resolveAnonymousChatContext(
  ...
  try {
    response = await postGate(stub, "/context", {});
  } catch (error) {
...
export async function reserveAnonymousChatSlot(
  ...
  let response: Response;
  try {
    response = await postGate(stub, "/lease", { requestId });
  } catch (error) {
...
export async function refundAnonymousChatSlot(
  ...
  const response = await postGate(gateStub(namespace, lease.credential.sessionToken), "/refund", {
...
export async function commitAnonymousChatSlot(lease: AnonymousChatGateLease): Promise<void> {
  ...
  const response = await postGate(gateStub(namespace, lease.credential.sessionToken), "/commit", {
    requestId: lease.requestId,
  });
  if (!response.ok) {
