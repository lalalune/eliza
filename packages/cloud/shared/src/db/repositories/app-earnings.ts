...
export class AppEarningsRepository {
  ...
  async applyCreatorMovement(
    ...
    if (
      ...
      throw new Error("Creator movement amounts must be finite and creator amount must be non-zero");
    }

    const expectedCreatorAmountRounded = expectedCreatorAmount.toDecimalPlaces(4);
    ...
    return dbWrite.transaction(async (tx) => {
      ...
      if (
        ...
        ledgerMetadata.appCreatorShadowVersion !== 1
      ) {
      ...
      if (!updatedEarnings) {
        throw new Error(`Insufficient app earnings balance for creator movement on ${params.appId}`);
      }
      ...
      if (!updatedApp) {
        throw new Error(`Insufficient app aggregate balance for creator movement on ${params.appId}`);
      }
