...
const MIGRATION_TAG = "0182_inference_authorization_revisions";
...
function migrationStatements(): string[] {
  return readFileSync(
    join(MIGRATIONS_DIR, `${MIGRATION_TAG}.sql`),
    "utf8",
  )
...
async function revisions(): Promise<{
  ...
  return {
    ...
    sessionNotBefore: Number(
      user.rows[0]?.inference_session_not_before,
    ),
...
describe("0182 inference authorization revisions", () => {
  test("is registered immediately after the accounting migrations", () => {
    ...
    const predecessor = journal.entries.find(
      (candidate) => candidate.tag === "0181_app_usage_projections",
    );
  ...
  test("advances organization, user, and credential revisions independently", async () => {
    await client.query(
      "UPDATE organizations SET name = 'Cosmetic rename' WHERE id = $1",
      [ORGANIZATION_ID],
    );
    await client.query("UPDATE users SET name = 'Cosmetic rename' WHERE id = $1", [
      USER_ID,
    ]);
    await client.query("UPDATE api_keys SET name = 'Cosmetic rename' WHERE id = $1", [
      API_KEY_ID,
    ]);
    expect(await revisions()).toEqual({
      organization: 0,
    ...
    await client.query("UPDATE users SET organization_id = NULL WHERE id = $1", [
      USER_ID,
    ]);
    ...
    await client.query(
      "UPDATE users SET steward_user_id = 'steward-migrated' WHERE id = $1",
      [USER_ID],
    );
