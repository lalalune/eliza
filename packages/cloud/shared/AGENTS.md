...
bun run --cwd packages/cloud/shared test                   # scripts/run-bun-tests.mjs (bun test --isolate, 60s default; win32: PGlite quarantine, #15785)
...
- **test timeout + win32 PGlite quarantine (#15785):** the package `test` command supplies the repository's 60-second per-test default before invoking `scripts/run-bun-tests.mjs` because Bun runs from this package directory; callers can still append an overriding `--timeout`. On Windows the wrapper runs the PGlite tenant-db placement-claimer suite in its own child `bun test` process and retries it (bounded) ONLY on a Bun native-crash signature (`panic(main thread): Illegal instruction`, exit 3), capturing the panic to `.tmp/bun-pglite-crash/` for the upstream Bun report (`scripts/bun-pglite-crash-upstream-report.md`). Genuine test failures never retry. Renamed the suite? Update `DEFAULT_QUARANTINED_SUITES` in `scripts/run-bun-tests-helpers.mjs` (the run fails loudly until you do).
