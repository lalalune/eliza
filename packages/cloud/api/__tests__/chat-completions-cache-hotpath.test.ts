/**
 * Proves the production Worker chat route reaches provider handoff using only
 * cache decisions and returns a retryable cold-cache response without joining
 * any authoritative dependency hydration.
 */
...
const cacheOnlyCatalogLookup = mock(async () => ({
  kind: "ready" as const,
  model: null,
  stale: false,
}));
...
const cacheOnlyPoolSelection = mock(async () => ({
  kind: "ready" as const,
  credential: null,
}));
