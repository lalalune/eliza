...
interface TestAuthorizationProof {
  readonly v: 1;
...
async function initializeAuthorization(
  authorization: TestAuthorizationProof,
): Promise<void> {
  const { organizationId, userId, credential } = authorization;
  expect(
    (
      await post(
          ...
          states: [
            {
              kind: "organization",
              id: organizationId,
              revision: authorization.organizationRevision,
              denied: false,
            },
            {
              kind: "user",
              id: userId,
              revision: authorization.userRevision,
              denied: false,
            },
            {
              kind: "moderation",
...
test("real Durable Object serialization prevents concurrent overspend", async () => {
  const authorization: TestAuthorizationProof = {
    v: 1,
    organizationId: "org-miniflare",
  ...
  await initializeAuthorization(authorization);
  expect(
    (
      await post("/hydrate", {
        balanceUsd: 10,
