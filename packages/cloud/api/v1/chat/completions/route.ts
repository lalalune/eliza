...
export async function handleChatCompletionsPOST(
  ...
  try {
    ...
    // do advertise a reasoning parameter in the catalog. Workers consume its
    // cache entry or return a retryable warming response; non-Worker tools keep
    // name-pattern fallback behavior.
...
async function tryPassthroughStreamingRequest(params: {
  ...
  try {
    ...
    // A transport failure cannot prove whether the upstream accepted and
    // processed the request before the connection was lost.
  ...
  if (!upstreamResponse.body) {
    ...
    return passthroughErrorResponse(503, "upstream provider response was incomplete");
  }
