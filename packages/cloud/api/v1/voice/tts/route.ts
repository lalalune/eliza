...
function operationClaimResponse(claim: VoiceTtsOperationClaim): Response | null {
  ...
  try {
    ...
    if (activeOperation) {
      if (!reservation.reservationTransactionId) {
        throw new Error("Keyed TTS reservation did not return a transaction id");
      }
    ...
    if (activeOperation) {
      ...
      replayAudioBytes = wav ?? (await drainAudioStream(audioStream));
    ...
    if (activeOperation && env.BLOB) {
      ...
      try {
        ...
        if (parsed && typeof parsed === "object" && !Array.isArray(parsed)) {
          failureBody = parsed as Record<string, unknown>;
        }
