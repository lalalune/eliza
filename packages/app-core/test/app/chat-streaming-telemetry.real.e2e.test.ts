...
const TOKEN_DELAY_MS = Number(process.env.MOCK_LLM_TOKEN_DELAY_MS ?? 1000);
...
async function startConversationServer(
  ...
  serverFrames: ServerFrame[],
): Promise<{ close: () => Promise<void>; server: Server; port: number }> {
  ...
  return { close, server, port: address.port };
}
...
suite("chat streaming telemetry real e2e", () => {
  ...
  afterAll(async () => {
    ...
    await state.closeApi();
    await state.cleanupRuntime();
    state.harnessServer.closeAllConnections();
