...
export const BIP39_WORD_SET: ReadonlySet<string> = new Set(BIP39_WORDLIST);

export const BIP39_WORD_INDEX: ReadonlyMap<string, number> = new Map(
	BIP39_WORDLIST.map((w, i) => [w, i]),
);

/** Valid BIP-39 mnemonic word counts (128–256 bits of entropy). */
 ...
 */
export function mnemonicValid(phrase: string): boolean {
	...
	for (let i = 0; i < entropyBytes.length; i += 1) {
		entropyBytes[i] = Number.parseInt(entropy.slice(i * 8, i * 8 + 8), 2);
	}
	const hash = createHash("sha256").update(entropyBytes).digest() as Uint8Array;
	let hashBits = "";
	for (let i = 0; i < Math.ceil(checksumBits / 8); i += 1) {
		hashBits += (hash[i] ?? 0).toString(2).padStart(8, "0");
	}
	return hashBits.slice(0, checksumBits) === checksum;
}
 ...
 */
export function findAllMnemonicPhrases(text: string): MnemonicSpan[] {
	const tokens = [...text.matchAll(/[a-zA-Z]{3,8}/g)];
	const spans: MnemonicSpan[] = [];
	let i = 0;
...
export function findMnemonicPhrase(text: string): string | null {
	return findAllMnemonicPhrases(text)[0]?.value ?? null;
}
