...
def _write_tiny_causal_lm(model_dir: Path) -> None:
    ...
    raw = tokenizers.Tokenizer(tokenizers.models.WordLevel(vocab=vocab, unk_token="[UNK]"))
...
def test_train_local_runs_real_forward_backward_and_scans_checkpoint(tmp_path: Path) -> None:
    ...
    assert report["floating_tensors"] > 0
    assert report["floating_elements"] > 0
