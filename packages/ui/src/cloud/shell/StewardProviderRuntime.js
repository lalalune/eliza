...
import { clearServerStewardSessionCookies, clearStaleStewardSession, configuredRefreshEndpoint, configuredSessionEndpoint, isPlaceholderValue, LocalStewardAuthContext, readStoredToken, tokenIsExpired, tokenSecsRemaining, } from "./StewardProviderShared";
...
function AuthTokenSync({ children }) {
    ...
    useEffect(() => {
        const syncToken = () => {
            ...
            fetch(configuredSessionEndpoint(), {
                ...
                if (body?.code === "server_secret_missing") {
                    console.warn("[steward] /api/auth/steward-session reports server-side secret missing - keeping localStorage token; cookie path will fail until the Worker is configured.");
                    return;
                }
                if (res.status !== 401) {
                ...
                if (current && !tokenIsExpired(current)) {
                    ...
                    lastSyncedToken.current = null;
                    console.warn("[steward] Session-sync 401 but stored token still valid — keeping it (likely a stale control-plane proxy)");
                    return;
                }
        ...
        const checkAndRefresh = async (force = false) => {
            const token = readStoredToken();
            if (!token)
                return;
            if (!force) {
                const secs = tokenSecsRemaining(token);
                if (secs !== null && secs >= REFRESH_AHEAD_SECS)
                    return;
            }
            if (refreshInFlight)
                ...
                try {
                    ...
                    if (res.status === 401) {
                        ...
                        if (!stored || tokenIsExpired(stored)) {
                            ...
                            console.warn("[steward] Refresh 401 but stored token still valid — keeping it (likely a stale control-plane proxy, not a revoked session)");
                        }
                    }
                }
                catch (err) {
    ...
    const localAuth = useMemo(() => ({
        ...
        verifyEmailCallback: async (token, email) => {
    ...
    return (_jsx(LocalStewardAuthContext.Provider, { value: localAuth, children: children }));
}
export default function StewardAuthRuntimeProvider({ apiUrl, children, tenantId, }) {
    const client = useMemo(() => new StewardClient({
        baseUrl: apiUrl,
    ...
    return (_jsx(StewardProvider, { client: providerClient, agentId: "eliza-cloud", theme: ELIZA_STEWARD_THEME, auth: authConfig, tenantId: tenantId && !isPlaceholderValue(tenantId) ? tenantId : undefined, children: _jsx(AuthTokenSync, { children: children }) }));
}
