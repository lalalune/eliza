 ...
 */
import { clearStoredStewardToken, STEWARD_REFRESH_ENDPOINT, STEWARD_SESSION_ENDPOINT, STEWARD_TOKEN_KEY, } from "@elizaos/shared/steward-session-client";
...
export function configuredSessionEndpoint() {
...
export function configuredRefreshEndpoint() {
    const apiBase = configuredApiBase();
    if (apiBase && !isPlaceholderValue(apiBase)) {
        if (!(isBrowserOnElizaHost() && isLocalhostApiBase(apiBase))) {
...
function stewardSessionClearUrls() {
    if (typeof window === "undefined")
        return [configuredSessionEndpoint()];
...
export function clearServerStewardSessionCookies() {
    for (const url of stewardSessionClearUrls()) {
        ...
        fetch(url, { method: "DELETE", credentials: "include" }).catch(() => undefined);
    }
}
...
export function tokenSecsRemaining(token) {
    const payload = decodeJwtPayload(token);
    if (!payload?.exp)
        return null;
    return payload.exp - Date.now() / 1000;
}
export function clearStaleStewardSession() {
    if (typeof window === "undefined")
        return;
