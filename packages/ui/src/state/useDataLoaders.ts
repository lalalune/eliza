...
function mergeLocalPendingConversationMessages(
  ...
  const serverTurnRoles = new Set(
    serverMessages.flatMap((message) =>
      message.clientMessageId
        ? [`${message.role}:${message.clientMessageId}`]
        : [],
    ),
  );
  // A server row resolves an optimistic row only when the client already
  // reconciled it to that exact durable id. Role/text/timestamp similarity is
  // ambiguous under repeated prompts and replies, so a reload preserves temp
  // rows until an exact completion receipt or an explicit failure owns them.
  const pendingMessages = currentMessages.filter((message) => {
    if (!isLocalPendingConversationMessage(message)) return false;
    if (serverIds.has(message.id)) return false;
    if (
      message.clientMessageId &&
      serverTurnRoles.has(`${message.role}:${message.clientMessageId}`)
    ) {
      return false;
    }
    return true;
