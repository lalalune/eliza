import { BACKGROUND_CATALOG_INDEX, DEFAULT_BACKGROUND_CATALOG_ID as SHARED_DEFAULT_BACKGROUND_CATALOG_ID, } from "@elizaos/shared/backgrounds/catalog-index";
 ...
 */
export const DEFAULT_SHADER_BACKGROUND_CONFIG = {
    mode: "shader",
 ...
 */
function gradientDataUrl(palette) {
    const [a, b, c] = [
    ...
    const svg = `<svg xmlns='http://www.w3.org/2000/svg' width='16' height='24' ` +
 ...
 */
export const BACKGROUND_CATALOG = BACKGROUND_CATALOG_INDEX.map((meta) => {
    if (meta.kind === "glsl") {
    ...
    if (meta.kind === "color") {
        return {
            ...meta,
            source: meta.palette[meta.palette.length - 1] ?? DEFAULT_BACKGROUND_COLOR,
            author: "curated",
        };
    }
    ...
    return {
        ...meta,
        source: meta.id === SHARED_DEFAULT_BACKGROUND_CATALOG_ID
 ...
 */
export const CURATED_NATURAL_BACKGROUNDS = BACKGROUND_CATALOG.filter((e) => e.kind === "image");
/** The animated GLSL catalog entries, mirrored from the shader preset library. */
export const GLSL_CATALOG_BACKGROUNDS = BACKGROUND_CATALOG.filter((e) => e.kind === "glsl");
/** The boot-default catalog id (re-exported from the shared index). */
export const DEFAULT_BACKGROUND_CATALOG_ID = SHARED_DEFAULT_BACKGROUND_CATALOG_ID;
/**
 ...
 */
export function resolveCatalogEntry(ref) {
    if (!ref)
        return undefined;
    ...
    if (byId)
        return byId;
    const byLabel = BACKGROUND_CATALOG.find((e) => e.label.toLowerCase() === needle);
    if (byLabel)
        return byLabel;
 ...
 */
export function catalogEntryToConfig(entry, resolveShaderSource) {
    if (entry.kind === "color") {
