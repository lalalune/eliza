 ...
 */
function isNativeCloudAgentSubdomain(url) {
    const parsed = parseUrl(url);
    if (!parsed)
        return false;
...
const nativeCloudHttpTransport = {
    async request(url, init, context) {
        ...
        if (isNativeCloudAgentSubdomain(url) &&
...
export function nativeCloudHttpTransportForUrl(url) {
    ...
    if (isNativeDirectCloudApiUrl(url))
        return nativeCloudHttpTransport;
    if (isNativeCloudAgentSubdomain(url))
        return nativeCloudHttpTransport;
    return null;
}
