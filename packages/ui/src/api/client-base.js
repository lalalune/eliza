 ...
 */
import { extractAssistantReplyText, SHELL_NAVIGATE_VIEW_WS_EVENT, stripAssistantStageDirections, } from "@elizaos/shared";
...
import { iosInProcessAgentTransportForUrl, isIosInProcessLocalAgentBase, } from "./ios-local-agent-transport";
...
const REPLAYABLE_WS_EVENT_TYPES = new Set([
...
export function isStreamGenerationError(value) {
    return value instanceof StreamGenerationError;
}
const CHAT_TURN_STATUS_KINDS = new Set([
    "thinking",
    "streaming",
    "running_action",
    "running_tool",
 ...
 *  not crashed on). */
function parseChatTurnStatus(parsed) {
    if (!parsed.kind || !CHAT_TURN_STATUS_KINDS.has(parsed.kind))
        return null;
    return {
        kind: parsed.kind,
...
function isElizaCloudControlPlaneBase(value) {
...
function parseStreamChatDataLine(line) {
...
function applyStreamChatTokenEvent(parsed, state, onToken) {
...
function applyStreamChatDataLine(line, state, onToken, onStatus) {
...
function isLocalAgentIpcBase(value) {
    const normalized = normalizeBaseUrl(value);
    if (!normalized)
        return false;
    return isMobileLocalAgentIpcUrl(normalized);
}
function isSharedRuntimeRestAdapterBase(value) {
    const normalized = normalizeBaseUrl(value);
    if (!normalized)
        return false;
    try {
...
function shouldTreatAsConnectedWithoutWebSocket(value) {
    return (isIosInProcessLocalAgentBase(value) ||
...
function isDedicatedCloudAgentBase(value) {
    const normalized = normalizeBaseUrl(value);
    if (!normalized)
        return false;
    try {
        const host = new URL(normalized).hostname.toLowerCase();
        return (host.endsWith(".elizacloud.ai") &&
...
function getInjectedWsBase() {
    if (typeof window === "undefined")
        return undefined;
    const values = [
        window.__ELIZA_WS_BASE__,
        window.__ELIZAOS_WS_BASE__,
    ];
...
function isMixedContentWebSocketBlocked(wsProtocol) {
    if (wsProtocol !== "ws:")
        return false;
    if (typeof window === "undefined")
        return false;
...
function isNetworkStatusChangeEvent(ev) {
    if (!("detail" in ev))
        return false;
...
if (typeof document !== "undefined") {
    document.addEventListener(NETWORK_STATUS_CHANGE_EVENT, (ev) => {
        if (!isNetworkStatusChangeEvent(ev))
            return;
        const next = ev.detail.connected;
        if (next === lastKnownNetworkConnected)
            return;
...
export function __getLastKnownNetworkConnected() {
...
function resumeRetryDelayMs(res) {
    const header = res.headers.get("Retry-After");
    const seconds = header !== null && Number.isFinite(Number(header))
        ? Number(header)
        : Number.NaN;
...
export class ElizaClient {
    ...
    _token;
    clientId;
    requestTransport = fetchAgentTransport;
    ws = null;
    wsHandlers = new Map();
    wsEventBacklog = new Map();
    wsSendQueue = [];
    wsSendQueueLimit = 32;
    reconnectTimer = null;
    backoffMs = 500;
    ...
    constructor(baseUrl, token) {
        ...
        const localStorageGetItem = typeof window !== "undefined" &&
    ...
    async rawRequest(path, init, options) {
        ...
        const token = this.apiToken ?? (await hydrateAndroidLocalAgentTokenForUrl(requestUrl));
        let res = await this.rawRequestOnce(path, requestUrl, init, options, token);
        if (res.status === 401) {
            const hydratedToken = await hydrateAndroidLocalAgentTokenForUrl(requestUrl, { force: true });
            const retryToken = hydratedToken ?? (!token ? this.apiToken : null);
            if (retryToken && retryToken !== token) {
                res = await this.rawRequestOnce(path, requestUrl, init, options, retryToken);
            }
        }
        ...
        if (!res.ok && !options?.allowNonOk) {
            ...
            const headerRetryAfter = headerValue !== null && Number.isFinite(Number(headerValue))
                ? Number(headerValue)
                : undefined;
            const rawBodyRetryAfter = body?.retryAfter;
            const bodyRetryAfter = typeof rawBodyRetryAfter === "number" &&
    ...
    async rawRequestOnce(path, requestUrl, init, options, token) {
        ...
        try {
            ...
            return this.throwRawRequestError(err, path, timeoutMs, timedOut, abortController);
        }
        finally {
    ...
    rawRequestInit(init, abortController, token) {
        return {
            ...init,
    ...
    async rawRequestTransport(requestUrl) {
        if (this.requestTransport !== fetchAgentTransport) {
            return this.requestTransport;
        }
        return ((await androidNativeAgentTransportForUrl(requestUrl)) ??
     ...
     */
    async readBodyText(res, path, timeoutMs, init) {
    ...
    // --- WebSocket ---
    rememberReplayableWsEvent(type, data) {
        if (!REPLAYABLE_WS_EVENT_TYPES.has(type))
            return;
    ...
    replayBackloggedWsEvents(type, handler) {
        const backlog = this.wsEventBacklog.get(type);
        if (!backlog?.length)
            return;
        const pending = backlog.slice();
        queueMicrotask(() => {
            if (!this.wsHandlers.get(type)?.has(handler))
                return;
     ...
     */
    sendWsMessage(data) {
        const message = typeof data.msgId === "string" && data.msgId.length > 0
            ? data
    ...
    normalizeAssistantText(text) {
        ...
        const stripped = stripAssistantStageDirections(extractAssistantReplyText(text) ?? text);
    ...
    async streamChatEndpoint(path, text, onToken, channelType = "DM", signal, images, metadata, 
    ...
    onStatus) {
        ...
        while (true) {
            ...
            try {
                ...
                const abortPromise = new Promise((_, reject) => {
                    ...
                    signal.addEventListener("abort", onAbort, { once: true });
                    void readPromise.finally(() => signal.removeEventListener("abort", onAbort));
                });
        ...
        const resolvedText = streamState.doneNoResponseReason === "ignored"
            ? ""
            : this.normalizeAssistantText(streamState.doneText ?? streamState.fullText);
        return {
            text: resolvedText,
