...
import { isMobileLocalAgentUrl as isConfiguredMobileLocalAgentUrl, isMobileLocalAgentIpcUrl, mobileLocalAgentPathFromUrl, } from "../first-run/mobile-runtime-mode";
import { handleIosLocalAgentRequest, startIosLocalAgentKernel, } from "./ios-local-agent-kernel";
...
import { headersToRecord, isStreamingRequest, } from "./transport";
let transport = null;
let globalRequestHandlerInstalled = false;
let globalFetchBridgeInstalled = false;
let originalFetch = null;
let fullBunRuntime = null;
 ...
 */
const TERMINAL_IOS_NATIVE_AGENT_BOOT_ERROR_FRAGMENTS = [
 ...
 * backend deadline.
 */
export function isTerminalIosNativeAgentBootErrorMessage(message) {
    if (!message)
        return false;
...
function resolveBootTraceBridge() {
    ...
    try {
        ...
        agentPluginForBootTrace = registerPlugin("Agent");
    }
    catch {
        agentPluginForBootTrace = null;
    }
 ...
 */
export function appendIosBootTrace(stage, detail = {}) {
...
export function recordIosNativeAgentBootPhase(phase, error) {
    const now = Date.now();
...
export function recordIosNativeAgentBootHeartbeat() {
    iosAgentBootProgress = {
        ...iosAgentBootProgress,
        lastHeartbeatAt: Date.now(),
    };
}
export function getIosNativeAgentBootProgress() {
 ...
 */
export function isIosNativeAgentBootInProgress(now = Date.now()) {
...
export function resetIosNativeAgentBootProgressForTests() {
    iosAgentBootProgress = {
        phase: "idle",
        startedAt: null,
        lastHeartbeatAt: null,
...
function isPrimedFullBunRuntime(value) {
    return (!!value &&
        typeof value === "object" &&
        value.kind === "primed");
}
...
function isFullBunRuntimeBuiltIn() {
    const env = viteEnv();
...
function shouldRequireFullBunRuntime() {
    const env = viteEnv();
...
function hasIosFullBunSmokeRequest() {
    try {
...
function readPersistedRuntimeMode() {
    try {
...
function fullBunStartupError(message, cause) {
...
function isNativeIosStoreBuild() {
    return isNativeIos() && isStoreBuild();
}
function isNativeIosCloudRuntime() {
    if (!isNativeIos())
        return false;
    const runtimeMode = readRuntimeMode();
...
function usesStrictIosNetworkPolicy() {
    return isNativeIosStoreBuild() || isNativeIosCloudRuntime();
}
function isLoopbackLocalAgentUrl(value) {
    try {
...
function allowsIosSimulatorLoopback(url) {
...
function isCleartextNetworkUrl(url) {
    return url.protocol === "http:" || url.protocol === "ws:";
}
function isIosLocalAgentIpcUrl(url) {
    return isMobileLocalAgentIpcUrl(url);
}
function isMobileLocalAgentUrl(value) {
 ...
 */
function iosRuntimeHasOnDeviceAgent() {
    const mode = readRuntimeMode();
    return (mode === "local" || mode === "cloud-hybrid" || mode === "tunnel-to-mobile");
}
function canUseIosLocalAgentIpc() {
    if (!isNativeIos())
        return false;
...
function localAgentPathnameFromUrl(url) {
...
function isCloudRuntimeAllowedLocalAgentPath(path) {
    const queryIndex = path.indexOf("?");
...
function wrapFullBunRuntime(runtime) {
    return {
...
export function isIosInProcessLocalAgentUrl(url) {
...
export function isIosInProcessLocalAgentBase(baseUrl) {
    if (!baseUrl)
        return false;
...
async function getFullBunRuntime() {
...
export function primeIosFullBunRuntime(runtime) {
...
async function importFullBunRuntimePlugin() {
    appendIosBootTrace("engine-import-start", { copy: "ui" });
    let mod = null;
    try {
        mod = (await import("@elizaos/capacitor-bun-runtime"));
    }
    catch {
        mod = null;
    }
    ...
    return wrapFullBunRuntime(mod?.ElizaBunRuntime ??
        registerPlugin("ElizaBunRuntime"));
}
async function tryFullBunNativeRequest(options) {
...
async function requestToNativeBridgeOptions(request, context) {
...
function nativeResultToResponse(result) {
 ...
 */
async function tryFullBunStreamingResponse(options) {
    const runtime = await getFullBunRuntime();
...
async function dispatchIosLocalAgentRequest(request, context) {
    ...
    return nativeResultToResponse(await handleIosLocalAgentNativeRequest(options));
}
let tracedNativeRequests = 0;
export async function handleIosLocalAgentNativeRequest(options) {
...
export function installIosLocalAgentNativeRequestBridge() {
...
function shouldBridgeFetchUrl(url) {
    if (!isNativeIos())
        return false;
    ...
    if (isMobileLocalAgentUrl(url.toString()))
        return true;
    if (isNativeIosCloudRuntime())
        return false;
...
function localAgentUrlForFetch(url) {
    const localAgentPath = mobileLocalAgentPathFromUrl(url.toString());
    if (localAgentPath)
        return `${IOS_LOCAL_AGENT_IPC_BASE}${localAgentPath}`;
    return `${IOS_LOCAL_AGENT_IPC_BASE}${url.pathname || "/"}${url.search}`;
}
export function installIosLocalAgentFetchBridge() {
...
export async function iosInProcessAgentTransportForUrl(url) {
