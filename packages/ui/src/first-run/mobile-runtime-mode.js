...
export function isMobileLocalAgentIpcUrl(value) {
    if (!value)
        return false;
    ...
    if (lower === MOBILE_LOCAL_AGENT_IPC_BASE ||
    ...
    try {
        ...
        return (pathname === `//${MOBILE_LOCAL_AGENT_IPC_HOST}` ||
...
export function isMobileLocalAgentIpcBase(baseUrl) {
    if (!baseUrl)
        return false;
...
export function mobileLocalAgentPathFromUrl(value) {
    if (!value)
        return null;
    const trimmed = value.toString().trim();
    if (!trimmed)
    ...
    if (lower === MOBILE_LOCAL_AGENT_IPC_BASE)
        return "/";
    if (lower.startsWith(`${MOBILE_LOCAL_AGENT_IPC_BASE}/`) ||
        lower.startsWith(`${MOBILE_LOCAL_AGENT_IPC_BASE}?`)) {
...
export function isMobileLocalAgentUrl(value) {
    if (!value)
        return false;
    if (isMobileLocalAgentIpcUrl(value))
        return true;
    let parsed;
    try {
...
export function normalizeMobileRuntimeMode(value) {
...
export function mobileRuntimeModeForServerTarget(target) {
    switch (target) {
        case "remote":
            return "remote-mac";
        case "elizacloud":
...
export function readPersistedMobileRuntimeMode() {
    if (typeof window === "undefined")
        return null;
    try {
        return normalizeMobileRuntimeMode(window.localStorage.getItem(MOBILE_RUNTIME_MODE_STORAGE_KEY));
    }
    catch {
...
async function persistNativeMobileRuntimeMode(mode) {
    try {
        ...
        logger.warn({ err, mode }, "[mobile-runtime-mode] native runtime-mode persist failed");
    }
}
/**
 ...
 */
export function persistMobileRuntimeMode(mode) {
    if (typeof window !== "undefined") {
        try {
            if (mode) {
            ...
            logger.warn({ err, mode }, "[mobile-runtime-mode] localStorage runtime-mode persist failed");
        }
    }
...
export function persistMobileRuntimeModeForServerTarget(target) {
