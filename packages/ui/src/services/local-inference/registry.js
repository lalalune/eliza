...
function normalizeStoredPathSeparator(target) {
    return target.split(/[\\/]+/).join(path.sep);
}
function storedRelativePath(target) {
    const root = path.resolve(localInferenceRoot());
    const resolved = path.resolve(target);
    if (!isWithinElizaRoot(resolved))
        return target;
    return path.relative(root, resolved).split(path.sep).join("/");
}
function reanchorLegacyAbsolutePath(target) {
    const resolved = path.resolve(target);
    if (isWithinElizaRoot(resolved))
        return resolved;
    ...
    const candidate = path.resolve(localInferenceRoot(), ...parts.slice(markerIndex + 1));
    return isWithinElizaRoot(candidate) ? candidate : resolved;
}
function resolveStoredElizaPath(target) {
    if (typeof target !== "string" || target.trim().length === 0)
        return null;
    if (path.isAbsolute(target) || /^[a-zA-Z]:[\\/]/.test(target)) {
        return reanchorLegacyAbsolutePath(target);
    }
    const resolved = path.resolve(localInferenceRoot(), normalizeStoredPathSeparator(target));
    return isWithinElizaRoot(resolved) ? resolved : null;
}
function hydrateStoredModel(model) {
    ...
    const manifestPath = model.manifestPath
        ...
        : undefined;
    if (model.bundleRoot && !bundleRoot)
        return null;
    if (model.manifestPath && !manifestPath)
        return null;
    return {
        ...model,
        path: modelPath,
...
function dehydrateStoredModel(model) {
    return {
        ...model,
...
async function readElizaOwned() {
    try {
        ...
        return parsed.models
            .filter((m) => m && typeof m === "object" && m.source === "eliza-download")
...
function externalScanEnabled() {
    const value = process.env.ELIZA_LOCAL_INFERENCE_ENABLE_EXTERNAL_SCAN?.trim().toLowerCase();
...
async function resolveRemovableElizaPath(target) {
    if (!isWithinElizaRoot(target))
