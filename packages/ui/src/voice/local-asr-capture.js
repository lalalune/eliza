 ...
 */
import { DEFAULT_POST_TTS_COOLDOWN_MS, isTtsEchoGateActive as sharedTtsEchoGateActive, } from "./tts-playback-activity";
...
export function isLocalAsrCaptureSupported() {
...
export function measurePcmAudio(pcm) {
    if (pcm.length === 0)
        return { rms: 0, peak: 0 };
    let sumSquares = 0;
    let peak = 0;
...
export function isSilentPcmAudio(pcm) {
...
export const DEFAULT_LOCAL_ASR_AUTO_STOP = {
    startGraceMs: 250,
    minSpeechMs: 180,
    silenceMs: 900,
    maxSpeechMs: 12_000,
...
export function createLocalAsrAutoStopDetector(options, startedAtMs = nowMs()) {
    if (!options)
        return null;
    const config = {
    ...
    const echoGateActive = options.isTtsEchoGateActive ??
        ((atMs) => sharedTtsEchoGateActive(atMs, cooldownMs));
    let firstSpeechAtMs = null;
    let lastSpeechAtMs = null;
    let stopped = false;
    return (pcm, sampleTimeMs = nowMs()) => {
        if (stopped)
        ...
        const gateMultiplier = echoGateActive(sampleTimeMs)
            ...
            : 1;
        const speechDetected = stats.rms >= config.speechRmsThreshold * gateMultiplier ||
            stats.peak >= config.speechPeakThreshold * gateMultiplier;
        if (speechDetected) {
            if (firstSpeechAtMs === null)
                firstSpeechAtMs = sampleTimeMs;
        ...
        if (speechDurationMs >= config.minSpeechMs &&
...
export function encodeMonoPcm16Wav(pcm, sampleRateHz) {
...
export async function startLocalAsrRecorder(options = {}) {
