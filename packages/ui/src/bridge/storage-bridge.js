 ...
 */
async function preferencesResponded() {
    let timeoutId = null;
    try {
        ...
        const probe = Preferences.get({ key: "eliza:first-run-complete" }).then(() => true);
...
async function readPreferenceWithTimeout(key) {
    let timeoutId = null;
    try {
        const timeout = new Promise((resolve) => {
 ...
 */
export async function initializeStorageBridge() {
    if (initialized) {
        return;
    }
    if (!isNativePlatform()) {
    ...
    for (let attempt = 0; attempt < PREFERENCE_HYDRATION_ATTEMPTS; attempt += 1) {
        ...
        if (attempt < PREFERENCE_HYDRATION_ATTEMPTS - 1) {
            await new Promise((resolve) => setTimeout(resolve, PREFERENCE_HYDRATION_RETRY_MS));
        }
    }
    ...
    const entries = await Promise.all(Array.from(SYNCED_KEYS, async (key) => [key, await readPreferenceWithTimeout(key)]));
        ...
        if (SYNCED_KEYS.has(key)) {
            ...
            setTimeout(() => {
                loadPreferences()
                    ...
                    logger.error({ err, key }, "[StorageBridge] failed to sync key to Preferences");
                });
            }, 0);
        }
    };
        ...
        if (SYNCED_KEYS.has(key)) {
            ...
            setTimeout(() => {
                loadPreferences()
                    ...
                    logger.error({ err, key }, "[StorageBridge] failed to remove key from Preferences");
                });
            }, 0);
        }
    };
 ...
 */
export function registerSyncedKey(key) {
    SYNCED_KEYS.add(key);
}
/**
 * Check if storage bridge is initialized
 */
export function isStorageBridgeInitialized() {
    return initialized;
}
