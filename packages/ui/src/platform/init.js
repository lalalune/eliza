 ...
 */
export function canSelectLocalRuntime() {
...
export function handleDeepLink(url, protocol, handlers) {
    let parsed;
    try {
        parsed = new URL(url);
    }
...
// ── Platform CSS setup ──────────────────────────────────────────────
export function setupPlatformStyles() {
...
export function injectPopoutApiBase() {
    const params = new URLSearchParams(window.location.search || window.location.hash.split("?")[1] || "");
    ...
    if (apiBase) {
        try {
            ...
            const allowPrivateHttp = /^10\.\d{1,3}\.\d{1,3}\.\d{1,3}$/.test(host) ||
    ...
    if (waifuAccessToken) {
        ...
        window.history.replaceState(window.history.state, "", removeUrlParameter(window.location.href, "waifu_access_token"));
    }
}
function removeUrlParameter(href, parameter) {
    ...
    if (hashQueryIndex >= 0) {
        ...
        const hashParams = new URLSearchParams(nextUrl.hash.slice(hashQueryIndex + 1));
