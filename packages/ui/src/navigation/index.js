...
import { Clock3, LayoutGrid, MessageSquare, Monitor, Phone, Radio, Settings, UserRound, Wallet, } from "lucide-react";
...
function viteEnvFlagEnabled(name, defaultValue) {
...
function walletLauncherTabs() {
    const tabs = listAppShellPages()
        ...
        .map((entry) => normalizePath(entry.path).toLowerCase() === "/inventory"
        ? (entry.tabAffinity ?? "inventory")
        : entry.id);
    return [...new Set(tabs.length ? tabs : ["inventory"])];
}
function hasAndroidTestFlag(search, hash) {
...
export function isAndroidPhoneSurfaceEnabled(detection = {}) {
    const search = detection.search ??
        (typeof window === "undefined" ? "" : window.location.search);
    const hash = detection.hash ??
        (typeof window === "undefined" ? "" : window.location.hash);
    if (hasAndroidTestFlag(search, hash))
        return true;
 ...
 */
export function isAospShellEnabled(detection = {}) {
    const search = detection.search ??
        (typeof window === "undefined" ? "" : window.location.search);
    const hash = detection.hash ??
        (typeof window === "undefined" ? "" : window.location.hash);
...
function getWindowNavigationLocation() {
    return typeof window === "undefined" ? undefined : window.location;
}
export function isAppWindowRoute(location = getWindowNavigationLocation()) {
    if (!location)
        return false;
    try {
...
export function shouldUseHashNavigation(location = getWindowNavigationLocation()) {
    if (!location)
        return false;
    return location.protocol === "file:" || isAppWindowRoute(location);
}
export function getWindowNavigationPath(location = getWindowNavigationLocation()) {
    if (!location)
        return "/";
...
export const ALL_TAB_GROUPS = [
    {
        ...
        tabs: ["chat"],
        icon: MessageSquare,
        description: "Conversations with your agent, inbound messages from every connector, and connector management",
    },
    {
...
export { SETTINGS_SECTION_META, } from "../components/settings/settings-section-meta";
...
const PATH_TO_TAB = new Map(Object.entries(TAB_PATHS).map(([tab, p]) => [p, tab]));
function normalizePathForLookup(pathname, basePath = "") {
...
export function pathForTab(tab, basePath = "") {
    const base = normalizeBasePath(basePath);
    const p = TAB_PATHS[tab] ?? `/${tab}`;
    return base ? `${base}${p}` : p;
}
export function isRouteRootPath(pathname, basePath = "") {
    return normalizePathForLookup(pathname, basePath) === "/";
}
export function resolveInitialTabForPath(pathname, fallbackTab, basePath = "") {
 ...
 *   links.
 */
export const LEGACY_PREFIX_TAB_ALIASES = {
 ...
 */
function prefixSubTabFromPath(normalizedPath) {
    return (PATH_TO_TAB.get(normalizedPath) ??
...
export function tabFromPath(pathname, basePath = "") {
    ...
    if (normalized === "/")
        return resolveDefaultLandingTab();
    ...
    const registeredAppShellPage = listAppShellPages().find((entry) => normalizePath(entry.path).toLowerCase() === normalized);
 ...
 */
export function getAppSlugFromPath(pathname, basePath = "") {
    const normalized = normalizePathForLookup(pathname, basePath);
    if (!normalized.startsWith("/apps/"))
        return null;
...
export { getMainTabApp, MAIN_TAB_FALLBACK, resolveDefaultLandingTab, } from "./main-tab";
