...
3. **Story gate (`audit:stories`, `test/story-gate/`).** Renders **every**
   ...
   changes. A story with `play` must declare `tags: ["interaction-required"]`;
   the source contract rejects missing/stale tags and the browser gate proves
   the tagged play function is prepared and finishes. Reusable helpers:
...
- **Builtin view mutations need semantic action twins.** First-party shell views
  ...
  sweep discovers every mutating page module and requires an authority mapping
  to semantic actions (`SETTINGS`, `SCHEDULED_TASKS`, `BACKGROUND`, etc.) or an
  explicit diagnostic exemption. When adding a button/filter/toggle/form
  handler to a builtin view, add or reuse the action first, then map the source
  file to that action. Stale and conflicting exemptions fail the same gate.
