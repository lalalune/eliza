...
test("changed nonstandard guarded tests fail instead of disappearing", () => {
  ...
  expect(workflow).toContain(
    "- name: Reject guarded-test manifest contract changes",
  );
  expect(workflow).toContain(
    "if: steps.changed.outputs.guarded_manifest_changes != ''",
  );
  expect(workflow).toContain(
    `printf '%s\\n' "\${{ steps.changed.outputs.guarded_manifest_changes }}"`,
  );
  expect(workflow).toContain(
    "The guarded-test debt manifest is sealed after its bootstrap commit",
  );
});
