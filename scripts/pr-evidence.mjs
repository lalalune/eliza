...
function createOverflowReleaseIfAbsent(tag) {
  ...
  try {
    ...
    if (!/already exists|already_exists|tag_name.*exist|Validation Failed/i.test(errText(err))) {
      throw err;
    }
  }
}
...
function uploadAssets(stagedByName, releases) {
  ...
  fail("could not upload evidence assets: every candidate pr-evidence release is full");
}
...
async function rows(pr, args) {
  ...
  if (localFiles.length > 0) {
    console.log(`Uploading ${localFiles.length} local file(s) to '${PRIMARY_RELEASE_TAG}':`);
