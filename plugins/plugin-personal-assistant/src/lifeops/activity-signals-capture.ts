 ...
 * for the app lifetime without the GUI shell mounting a React effect. Returns a
 * stop function that removes every listener/interval it installed.
 */
...
export function startLifeOpsActivitySignalCapture(enabled = true): () => void {
  ...
  const refreshRuntimeReady = async (): Promise<boolean> => {
    try {
      ...
      // error-policy:J4 status probe failure means the runtime is not reachable
      // yet; capture stands down (false) until the next poll succeeds.
      runtimeReady = false;
      return false;
    }
  };
