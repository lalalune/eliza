...
- `proof-surface` — the npm-style package name. Use lowercase, hyphenated. Scope under `@elizaos/plugin-` for first-party plugins, otherwise pick a sensible scope (`@user/plugin-foo`).
- `Proof Surface` — the human-readable display name shown in the dashboard.
...
```
PLUGIN_CREATE_DONE {"pluginName":"<proof-surface>","files":["<list>"],"tests":{"passed":<n>,"failed":0},"lint":"ok","typecheck":"ok","description":"<one factual sentence>"}
```
