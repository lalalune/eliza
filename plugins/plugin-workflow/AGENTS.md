...
plugins/plugin-workflow/
  ...
  src/
    ...
    services/
      ...
      workflow-execution-error.ts  Public, secret-safe durable error envelope
    providers/
...
- **Idempotency keys and schedule identity.** `WorkflowDispatchOptions.idempotencyKey` prevents duplicate executions when the same trigger fires concurrently. Scheduled tasks include the stable schedule-node id in that key and pass it through the pending execution so one schedule fires only its own reachable branch.
...
- **Scale-to-zero subscription boundary.** Active clock-driven triggers (`once`, `interval`, and `cron`) require `dedicated-always` hosting on managed Cloud. Workflow activation, `/api/triggers` create/update, the chat `TRIGGER` action, and scheduler delivery all enforce the same `workflow_requires_always_on` contract. Disabled drafts, inbound event triggers, local runtimes, and explicit manual runs remain available; the UI must explain continuous credit usage before linking to the existing always-on confirmation flow.
- **Smithers orchestrator.** Workflow node execution delegates to `smithers-orchestrator@0.28.0` (see `src/services/smithers-runtime.ts`). The `effect` and `quickjs-emscripten` packages support the orchestrator's functional pipeline and sandboxed JS evaluation respectively. Definitions and trigger data cross a dedicated pipe rather than the worker environment, runs are time-bounded, and failures cross the worker/persistence/API boundaries only through the fixed envelope in `workflow-execution-error.ts`; raw messages, stacks, response bodies, and arbitrary context never become durable execution errors. The workflow database owns a separate renewable execution lease whose TTL exceeds the Smithers deadline and is fenced immediately before every node; startup recovery atomically claims only expired leases, including when replicas use host-local Smithers SQLite. Recovery is at-least-once after the previous owner is known dead, not universal exactly-once delivery; remote connectors still need their own idempotency support.
