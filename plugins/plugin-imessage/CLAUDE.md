...
- **Bun:sqlite / node:sqlite dual runtime.** `chatdb-reader.ts` tries `bun:sqlite` first, then `node:sqlite`, always with the backend's read-only flag. If chat.db can't be opened, the live service logs and enters its send-only state.
- **Strict backfill boundary.** `pageMessages`, `getLatestRowIdStrict`, and `listChatsStrict` throw on closed/query/attachment failures so exporters cannot confuse failure with an empty history. `snapshotChatDb` invokes SQLite's online backup with a read-only source, rejects control-character paths, and streams the published snapshot digest.
