...
- **Embedding truncation.** Inputs are counted with `models.countTokens` and oversized text is reduced on Unicode code-point boundaries until the provider verifies it fits the limit selected by `getEmbeddingInputTokenLimit` (`gemini-embedding-001` -> 2 048 tokens; `gemini-embedding-2` -> 8 192). Character length and the telemetry-only `length / 4` estimate are never correctness boundaries.
